...
import scalation.calculus.Calculus.{FunctionV2S, gradient1, gradient, gradientD}
      ...
      extends Minimizer with Error
{
    ...
    private val BETA     = 1.5             // increase factor for the weight
    ...
    {
        var x:  Pair = (x0, gradient1 (fg, x0, 1.0))       // current (point, gradient)
...
//      b    = eye (x0.dim)                           // approx. Hessian matrix (either use b or binv)
        ...
        for (k <- 1 to MAX_ITER if x._2.normSq > TOL) {
            ...
            println ("step = " + s)
            xx = (x._1 + s, gradient1 (fg, x._1 + s, 1.0))  // compute the next point
            println ("next pt = " + xx._1)
            if (bfgs) {
