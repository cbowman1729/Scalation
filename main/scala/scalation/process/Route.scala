...
import scalation.random.{Bernoulli, Discrete, Uniform, Variate}
      ...
      extends Component
{
    ...
    private var _turn: Component = null
    ...
    def select: Int = lane(0).select 

    def setSelector (selectorRV: Variate = Discrete (VectorD (0.25, 0.5, 0.25))) { lane(0).setSelector (selectorRV) }

    def setTurn (turn: Component) { _turn = turn }
