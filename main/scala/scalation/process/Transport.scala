      ...
      extends Component
{
    ...
    private var selector: Variate = Discrete (VectorD (0.25, 0.5, 0.25))

    def select: Int = selector.igen

    def setSelector (selectorRV: Variate) { selector = selectorRV }
...
//      director.animate (this, CreateEdge, blue, QCurve (), from, to, Array (bend))
    ...
    /** Jump the entity (SimActor) down this Transport.  Place it in the middle
     *  of the Transport/Edge/QCurve for the entire trip time.  
     *  @param dur  the duration of the trip
     */
    def jump (dur: Double)
    {
        val actor    = director.theActor
        ...
        accum (onTransport)
        onTransport += 1
        trace (this, "jumps for " + dur, actor, director.clock)
...
//        println ("dur = " + dur)
        ...
        accum (onTransport)
        onTransport += 1
        trace (this, "moves for " + dur, actor, director.clock)
        ...
        for (i <- 1 to steps) {
            if (loc != null) {
                director.animate (actor, MoveToken, null, null, Array (loc.x, loc.y))
                actor.schedule (dur / steps.toDouble)
                actor.yieldToDirector ()
//              println ("Transport.move: -- before loc = " + loc)
                curve.setTrajectory (actor.trajectory)
                loc = curve.next (DIAM, DIAM)
