...
//  private val agenda = new PQueue [SimActor] ()
     ...
     */
    private var stopTime = MAX_VALUE
     ...
     */
    def stopped: Boolean = _clock > stopTime

    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Set the stop time from the model.
     */
    def setStopTime (sTime: Double) { stopTime = sTime }
...
//            for (p <- parts) if (p.isInstanceOf [Gate]) println (p.name + ": " + p.asInstanceOf [Gate].num)
...
//      for (st <- stats) println ("getStatistics: " + st.show)
    ...
    {
        if (!_reportsOff) report                          // report in terminal
        if (animating) {
            if (!_reportsOff) reportf                     // report in new window
